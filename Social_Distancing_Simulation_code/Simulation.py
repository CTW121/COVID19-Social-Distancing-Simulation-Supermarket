...
class Counter:
    counter_img = 0 # for output figure numbering

class Simulation:
    def __init__(self, seed, Lx,Ly, nShelves, Diffcoeff, Sinkcoeff, Plumemin, PlumeConcInc, PlumeConcCont, nExits, Cashierd, EntracePos, ExitPos, nCustomers=1, probNewCustomer=0.1, probInfCustomer=0.05, nPlumes=20, maxSteps=1000, outputLevel=0, importGeometry=1, useDiffusion=0, imageName="test.pbm", dx=1.0):

        print("NEXITS: ", nExits)
        print("CASHIERD: ", Cashierd)
        print("ENTRANCEPOS", EntracePos)
        print("EXITPOS: ", ExitPos)
        print("nCustomers: ", nCustomers)

        self.nExits = nExits
        self.Cashierd = Cashierd
        self.EntracePos = EntracePos
        self.ExitPos = ExitPos

        self.Diffcoeff = Diffcoeff
        self.Sinkcoeff = Sinkcoeff
        self.Plumemin = Plumemin
    ...
    ## two running counters to keep measure of things
        ...
        self.store = Store(Lx,Ly, dx, Diffcoeff, Sinkcoeff, Plumemin, nExits, Cashierd, EntracePos, ExitPos)
        ...
        self.exposureHistTimeThresLevel01 =np.zeros(self.nCustomers)
        self.exposureHistTimeThresLevel02 =np.zeros(self.nCustomers)
        self.exposureHistTimeThresLevel03 =np.zeros(self.nCustomers)
        self.exposureHistTimeThresLevel04 =np.zeros(self.nCustomers)
        self.exposureHistTimeThresLevel05 =np.zeros(self.nCustomers)
    ...
    def newCustomer(self):
        ...
        if np.random.rand()<self.probInfCustomer:
            ...
            infected = 0
        new = SmartCustomer(self.store.entrance[0],self.store.entrance[1], self.PlumeConcInc, self.PlumeConcCont, infected=infected)
    ...
    def printStore(self, step, title):
        ...
        if not self.useDiffusion:
            ...
            X,Y = np.meshgrid(x,y)
            plumePlotData = (np.transpose(z.reshape(self.store.Lx,self.store.Ly)))          # IMPORTANT: plume data
            # print("plumePlotData: ", plumePlotData)
            ##np.savetxt('{:07d}.txt'.format(step), plumePlotData)                          # Write the plume plot data in a text file in every step
        ...
        #cb = fig.colorbar(difPlumes, cmap=self.parula_map, norm=mpl.colors.LogNorm(vmin=0.1,vmax=100.0), extend='min', ticks=[0.1, 1.0, 10.0, 100.0], orientation="horizontal")
        ...
        plt.savefig("{:07d}.png".format(step))
        
        #if step%20 == 0:
        #    plt.savefig("{:07d}.png".format(Counter.counter_img))
    ...
    def printEndStatistics(self):
        ...
        header = "seed {}, storedims {}x{}, probNewCustomer {}, probInfCustomer {}, steps {}, avrCustomersInStore {}, NEXITS {}, PROBSPREADPLUME {}".format(self.seed,self.store.Lx, self.store.Ly, self.probNewCustomer,self.probInfCustomer, self.maxSteps, np.mean(self.customersNowInStore),self.nExits, PROBSPREADPLUME)
        if not self.useDiffusion:
            header+=" PLUMELIFETIME {}".format(PLUMELIFETIME)
        else:
            #header+=" PLUMECONCINC {}, DIFFCOEFF {}, SINKCOEFF {}".format(PLUMECONCINC,self.DIFFCOEFF,ACSINKCOEFF)
            header+=" PLUMECONCINC {}, DIFFCOEFF {}, SINKCOEFF {}".format(self.Plumemin, self.Diffcoeff, self.Sinkcoeff)
        ## data per customer	
        dataArr = np.array([self.customerInfected[:self.customerNow], self.itemsBought[:self.customerNow], self.exposureHist[:self.customerNow], self.timeSpent[:self.customerNow], self.exposureHistTime[:self.customerNow], self.exposureHistTimeThres[:self.customerNow], self.exposureHistTimeThresLevel01[:self.customerNow], self.exposureHistTimeThresLevel02[:self.customerNow], self.exposureHistTimeThresLevel03[:self.customerNow], self.exposureHistTimeThresLevel04[:self.customerNow], self.exposureHistTimeThresLevel05[:self.customerNow]]).T
        #np.savetxt("customer_data_{}.dat".format(self.seed), dataArr, header=header)
        np.savetxt("customer_data.dat", dataArr, header=header)
        ...
        np.savetxt("store_data.dat", dataArr, header=header)
        #np.savetxt("integrated_plumes_store_data_{}.dat".format(self.seed), self.store.plumesIntegrated, header=header)
        np.savetxt("integrated_plumes_store_data.dat", self.store.plumesIntegrated, header=header)
    ...
    def runSimulation(self):
        ...
        approxOutFlux = 1.0/(0.5*(MAXSHOPPINGLIST+1)+1)*self.nExits 
        print("Approx influx: {} customers / s --- Approx maximum outflux: {} customers / s"
              .format(self.probNewCustomer, approxOutFlux))
        ...
        for i in range(self.maxSteps):
            ...
            for ind in customersExit[::-1]:
                ...
                ti,tx,ty,tz, tw, twt, l01, l02, l03, l04, l05 = leavingCustomer.getFinalStats()
                ...
                self.exposureHistTimeThresLevel01[self.customerNow]=l01
                self.exposureHistTimeThresLevel02[self.customerNow]=l02
                self.exposureHistTimeThresLevel03[self.customerNow]=l03
                self.exposureHistTimeThresLevel04[self.customerNow]=l04
                self.exposureHistTimeThresLevel05[self.customerNow]=l05
            ...
            if self.outputLevel or customersHeadExit>maxQueue:
                if i%60 == 0:
                    ...
                    self.printStore(Counter.counter_img, title=stepStr)
                    Counter.counter_img += 1
